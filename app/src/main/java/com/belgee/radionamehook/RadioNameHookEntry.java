...
public class RadioNameHookEntry implements IXposedHookLoadPackage {
    ...
    private void hookWidgetMethods(Class<?> targetClass, final String classNameForLog) {
        ...
        // Перехватываем их по имени (любая сигнатура) и через reflection переписываем поле.
        for (Method m : targetClass.getDeclaredMethods()) {
            String mn = m.getName();
            if (mn.equals("setMusicPlayInfoAndNotify")
                || mn.equals("updateMusicPlayInfo")
                || mn.equals("setMediaContentInfoAndNotify")) {
    ...
    private void hookHolderUpdateMethod(final Method method, final String classNameForLog) {
        try {
            // Получим сигнатуру для логирования
            ...
            XposedBridge.hookMethod(method, new XC_MethodHook() {
                ...
                protected void afterHookedMethod(MethodHookParam param) {
                    ...
                    try {
                        ...
                        // Перезаписываем только если поле пустое
                        ...
                        XposedBridge.log(TAG + ": holder update hook error: " + t.getMessage());
                    }
                }
            });
            XposedBridge.log(TAG + ":   ✓ hooked " + method.getName()
                + "(" + sigStr + ") on " + classNameForLog);
        } catch (Throwable t) {
            XposedBridge.log(TAG + ": hook " + method.getName() + " failed: " + t.getMessage());
        }
    }
